...
A public `pico_main_agent_create` with `PICO_SESSION_RESUME` remains **synchronous**: it validates and replays the complete file before returning. Do not call it from a per-frame extension callback for a large session; it can block that frame. Sidebar stored-session selection, `/resume`, and CLI startup use a separate private asynchronous loader. That loader keeps the current chat selected until its worker validates the file and main-thread replay finishes; selecting another chat or requesting another load cancels the prior UI load. Failed validation does not replace the old agent. A private candidate that has received callbacks gets an agent-destroy hook if its load is cancelled. This private UI path does not add a pending result to the public API.
...
`pico_agent_active` / `pico_agent_select` are UI selection only; they do not retarget an in-progress submit, cancel, model, effort, session, or compaction action. Composer submit and slash commands snapshot the selected ID once and pass it into those APIs. Selection clears transcript selection/scroll snapshots but leaves the global composer draft unchanged. Selecting a different agent, or creating a selected main agent, closes an unused pending main draft: a durable main agent with no session identity yet. Footer and `/model` `/effort` assign the JSONL identity on the settings call (the header and `model_change` may land a moment later); after that the agent is kept. Permanent `PICO_SESSION_NONE` agents are not discarded.
