...
`PicoPlugins_Reload` queues while any live or retired runtime has provider/tool work, pending calls, an offered-tool snapshot, a pending ask, undrained events, retained extension callbacks, or a delegation wait/job. A `.c` or subagent-profile file written in this turn does not load until that work finishes. Polling is ~0.5s and detects source content changes; F5 and `/reload` request the same path. As soon as reload is queued, Pico refuses new external turns and delegations while continuing to pump existing follow-ups, cancellation, and asks. Once quiescent, Pico compiles and initializes a complete candidate module/registration generation before replacing the active one. Failed compilation, loading, validation, or initialization leaves the previous generation and instance state active. Retained runtimes release their exact immutable registration generation only after their callbacks and queued events finish. Restricted live agents are revalidated, each live session is announced, and structured tool details are replayed.

`/cd` is also queued, never synchronously waits, and uses the same barrier. The live workspace path remains unchanged while old agents drain. Pico then destroys the old agent set, switches workspace/settings/registrations, revalidates user-global profiles, and publishes one fresh main agent as one main-thread transition.
...
- `/cd` queues a deferred workspace replacement: current work drains, then Pico creates a new session, loads workspace settings/plugins, and replaces the live workspace. The workspace path is immutable after open. Assigning a path field does not perform a transition.
