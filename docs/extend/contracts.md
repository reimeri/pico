...
Read this before writing an extension. Getting these wrong crashes Pico or silently no-ops. Host and workspace lifecycle details are in [host](host.md) and [workspace](workspace.md).

## Reload and workspace quiescence

Workspace reload queues while that workspace has a live or retired runtime with provider/tool work, pending calls, an offered-tool snapshot, a pending ask, undrained events, retained extension callbacks, or a delegation wait/job. A `.c` or subagent-profile file written in this turn does not load in that workspace until that work finishes. Polling is ~0.5s and detects source content changes. F5 and `/reload` reload host extensions immediately (user-global / config sources only) and request reload of the selected agent's workspace only; other workspaces keep accepting work. A compile or validation failure in one workspace's local extensions does not block host-extension replacement. As soon as a workspace reload is requested, that workspace refuses new external turns and delegations while continuing to pump existing follow-ups, cancellation, and asks. Host-extension replacement does not wait for workspace workers. Once a workspace is quiescent, Pico compiles and initializes a complete candidate module/registration generation before replacing the active one. Failed compilation, loading, validation, or initialization leaves the previous generation and instance state active. Retained runtimes release their exact immutable registration generation only after their callbacks and queued events finish. Restricted live agents are revalidated, each live session is announced, and structured tool details are replayed.

`/cd` canonicalizes and opens the target workspace (or reuses an already-open canonical path), creates a main agent there if none exists, and selects a main agent through `pico_agent_select`. Relative paths resolve against the command agent's workspace, not UI selection. A newly opened workspace is closed if creating its first main agent fails. `/cd` does not select into a closing workspace. The previous workspace stays open and keeps pumping.
...
On process exit, every agent, delegation job, and retired runtime shares one absolute deadline of about one second. `pico_host_free` returns `PICO_HOST_SHUTDOWN_CLEAN` when all workers join. If any callback remains blocked, it returns `PICO_HOST_SHUTDOWN_RETAINED`: Pico detaches it and retains the heap execution host plus every registration, auth store, builtin state, and user-extension `.so` handle it can reach. No extension `shutdown`, `dlclose`, retained auth destruction, or curl cleanup occurs. Pico is then permanently retired in that process; later `pico_host_init` and plugin lifecycle attempts are rejected, and the caller must proceed to process exit. Host UI state is never worker-owned. Only `pico_host_free` applies this process deadline; workspace close does not.

`--safe` skips user extensions. Compile errors and failed `pico_add_tool` registrations set the host overlay warning. `pico_status_warn` appends a line to that overlay.

## Threads

Main thread: `init`, `shutdown`, `on_frame`, view render, notification hooks, after-tool hooks, tool-row hooks, tool apply callbacks, LLM/context hooks, command `run`, completer query/accept, auth login/logout, `pico_ui_modal_push` / `pop`, `pico_agent_ui_latest` / `pico_agent_ui_clear`. Agent-scoped callbacks receive a `PicoAgentId`; keep mutable agent/session extension state in an ID-keyed map. Main-thread callbacks are serialized and must return promptly. They must not join or wait for asynchronous work. A user `.so` must not leave an extension-owned background thread running across frames: the public lifecycle has no pre-shutdown quiescence callback, `shutdown` cannot wait, and `dlclose` follows it. A stuck main-thread callback blocks the whole host and is an extension contract violation. View render callbacks are declarative and may run more than once per displayed frame during same-frame reflow; keep durable state changes, I/O, and input consumption in `on_frame` or hooks.

Worker thread: `PicoToolFn`, `PicoToolBeforeFn`, `PicoProviderStreamFn`. They receive a callback-scoped opaque `PicoAgentContext *`, never `PicoHost *`. Do not retain it. Worker callbacks from different agents and workspaces may overlap and must be reentrant. Use context accessors, `pico_tool_ask`, `pico_ui_post`, `pico_tool_set_child`, and `pico_auth_copy_ctx`; do not touch UI, transcript, session, settings, model catalog, or unsynchronized agent-scoped extension state. `pico_agent_context_workspace` / `pico_agent_context_workspace_id` are the immutable path and ID copied when the turn was accepted, not the UI-selected workspace. `pico_agent_context_registration_generation` is the workspace registration generation copied for that same accepted turn. `pico_tool_ask` and `pico_ui_post` may be called only from a tool or before-tool callback. Do not block on your own condition variable — Esc, force-cancel, reload, and shutdown cannot wake it.
...
Named delegation is a worker/main-thread handshake: the parent tool waits on a core-owned job without holding workspace locks; the main thread creates and pumps the child. Worker callbacks may therefore run for a child while its parent worker is blocked. Child asks are still published through the normal global ask routing. The builtin chat inspects a child from its parent `subagent` tool row without selecting that child.

Pico and its extensions never call `chdir()` in the host process. Shell tools may change directory only after `fork()` in the child.
...
- `shutdown` must not wait for worker completion. User extensions must have no live extension-owned threads when `shutdown` is invoked; `dlclose` follows immediately. Core-owned worker callbacks are quiesced by Pico before workspace shutdown. Builtins compiled into the host executable may use core-managed detached lifetime, but that pattern is not safe for a user `.so`.
...
`PicoPlugins_Count(host)` / `PicoPlugins_Get(host, index, out)` return one record per active host or workspace slot (builtins and user sources, including failed loads). Pointers in `PicoExtInfo` are valid until the next reload. `scope` is `PICO_EXTENSION_HOST` (workspace ID zero) or `PICO_EXTENSION_WORKSPACE` (explicit workspace ID). `loaded` is true when compile/`dlopen` succeeded (builtins always). `enabled` is independent: a loaded extension is off when its `name` is in host `host_preferences.json` `disabled_host_extensions` or workspace `settings.json` `disabled_extensions`. Disabled extensions skip `init` and `on_frame`; `shutdown` runs only if that extension was initialized. Failed stubs are not enabled. The builtin `extensions` manager cannot be disabled. Click a row in `/extensions` (or F2) to toggle; Pico reloads to drop or restore registrations.

## Limits

- 8 live workspaces (`PICO_MAX_WORKSPACES`)
- 16 agents per workspace (`PICO_MAX_AGENTS`), 32 agents host-wide (`PICO_MAX_TOTAL_AGENTS`)
- 32 user `.c` files
- 16 views per slot
...
- 16 UI mailbox keys per workspace (`PICO_MAX_UI_POSTS`), keyed by `(agent_id, runtime_generation, name)`; names shorter than `PICO_UI_MODAL_NAME`
- `PICO_UI_POST_TEXT_MAX` (64 KiB) and `PICO_UI_POST_STATUS_MAX` (128 bytes)
- 16 completers, providers, auth registrations
- 24 completion items per query; completion labels are at most `PICO_COMPLETE_LABEL_MAX` (288) UTF-8 bytes
...
- Host preferences: `~/.config/pico/host_preferences.json`
- User-global models/defaults: `~/.config/pico/settings.json`
- Workspace settings: `<workspace>/.pico/settings.json`
- Session catalog: `~/.config/pico/sessions/<encoded-key>/` (or `$XDG_CONFIG_HOME/pico/sessions/`). Each directory is one catalog workspace. Parent `*.jsonl` files are sessions; subagent jsonl is omitted from `/resume` and the sidebar. `.workspace.json` stores required `path` (encoded keys are not reversible), display `name`, `order`, `collapsed`, and a per-session `model`/`effort` overlay. jsonl is authoritative for which sessions exist and for model/effort on disk (header plus last `model_change`). Pico rewrites missing metadata from jsonl `cwd` on scan. Scan omits a catalog workspace whose `path` is not an existing directory.
...
- `/cd` opens or selects a workspace: the target is canonicalized relative to the command agent's workspace, an already-open path is reused, a main agent is created only if that workspace has none, and selection moves there via `pico_agent_select`. A newly opened workspace is rolled back if that create fails. Closing workspaces are rejected. The previous workspace is not closed or paused. The workspace path is immutable after open.

## `PicoHost` and `PicoWorkspace`

`PicoHost` and `PicoWorkspace` are opaque. Prefer `pico_add_*` / `pico_host_add_*` / `pico_workspace_add_*` and the host setters listed in the topic pages (`pico_host_request_submit_cancel`, `pico_host_set_agent_input`, `pico_host_set_agent_parts`). Conversation/runtime/session/model/usage fields live in opaque workspace-owned `PicoAgent` instances. Use the copied `pico_agent_*` snapshots. See [host](host.md), [workspace](workspace.md), and [agents](agents.md). The private app-level `agent.h`, `agent_internal.h`, `host_internal.h`, `workspace_internal.h`, `session.h`, and `settings.h` are not extension API.

The model catalog is resolved per workspace from user-global configuration plus `<workspace>/.pico/settings.json` and is immutable while agents in that workspace run. Each agent owns copied model/effort/context/compaction selection, so changing defaults or replaying a session does not mutate another live agent. `context_limit` comes from the selected catalog model when that model has one; `context_limit_fallback` in workspace settings / `PICO_CONTEXT_LIMIT` is only a fallback.

`pico_session_log_custom(host, agent_id, "myext", "{…json…}")` appends a JSONL record to the explicit target on the main thread and returns `PICO_SESSION_WRITE_SKIPPED`, `_OK`, or `_FAILED`. Skipped means the target is intentionally ephemeral. An open/write/fsync/close failure moves the target to `PICO_SESSION_FAILED`, rolls back a partial final line when the file supports truncation, preserves its ID/path and reservation until reset/close, warns in the overlay, and makes `PicoAgentInfo.resumable` false. Stale targets fail. Session replay does not dispatch custom records back to extensions. For replayable tool-owned state, return validated structured `details_json` and register a tool apply callback instead. Apply receives the target ID. Details are replayed chronologically on resume and extension reload; use complete snapshots and make replay application idempotent.

Zero is invalid for every ID. Stale workspace, agent, ask, and runtime-generation IDs fail without affecting a different object.

## Sessions and delegation

Session JSONL schema version 4 requires a `kind` in every header. A subagent header also requires durable `profile` and `initial_purpose` metadata and may include `parent_session_id`. The header may include `title`, the `/resume` label; when it is absent Pico derives that label from the first user message. Each changed title appends a `title` event and rewrites the header `title` in one persistence operation; setting the existing title is a no-op. The builtin TODO state reconciles its task title after durable replay, so a process exit between the tool-result write and the next frame cannot leave the label stale. Session appends and title rewrites wait on the same per-session inter-process lock. A rewrite uses a unique temporary file, syncs its contents, atomically replaces the session, and syncs the containing directory before reporting success. Replay does not dispatch title events. Every assistant-message and tool-call event carries a non-negative `message_group`; consecutive events with the same group restore into the same assistant bubble, while a changed group starts a new bubble. This preserves the live transcript independently of provider item boundaries. Assistant events may include `thinking_parts`, an ordered array of reasoning-summary steps whose final entry is the widget title. Events with raw `thinking` or structured `thinking_parts` may also include `thinking_ms` (elapsed time of that reasoning burst). Resume restores the steps and duration onto the think line so expanded summaries and duration labels survive. Pico does not replay older schemas. One workspace reservation owns each open session path, and failed replay releases the reservation without publishing an agent. Session directory resolution accepts the target workspace; it never reads UI selection or process CWD. A `PICO_SESSION_NEW` main agent has no file until the first JSONL write. Selecting another agent, or creating a selected main agent, closes that unused draft; a write makes the identity durable and the agent is then kept.
...
Delegation jobs and child/session ownership survive force cancellation until every worker, workspace, and child reference releases them. Parent cancellation wakes the waiting generation immediately and requests child cancellation. A late child terminal event cannot publish to a replacement parent generation. A subagent and every descendant belong to the parent's workspace.

## Tool authorization

Each request starts from registered tools, applies the agent policy, then LLM-hook exclusions. For a named child, the policy is the current profile's exact allowlist snapshot; omission allows all tools and an empty array allows none. Continued sessions refresh this policy instead of restoring an old catalog. Context hooks inspect the final effective catalog. The provider receives a retained snapshot, and tool execution/apply resolve only from that snapshot—not from the live registry. A hidden or unoffered call becomes a logged tool error and bypasses before hooks, tool code, apply, and after hooks. Empty/duplicate call IDs, malformed call arrays, and calls beyond the pending-call limit fail the provider round explicitly. Pico executes the pending calls from one provider round one at a time; later calls remain queued until earlier ones finish.
