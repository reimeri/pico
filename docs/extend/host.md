...
The normal CLI startup initializes the host, opens the initial directory, creates and selects one main agent, then schedules optional session resume. File reading and strict JSONL validation run on a core worker; the host pump replays records on the main thread in bounded batches and replaces the initially selected agent only after replay completes. Until then the empty startup agent cannot submit a turn. Failed startup resume leaves that agent available and reports an error. An extension holding the initial agent ID must not assume that ID survives successful startup resume. The installed desktop launcher passes `--no-workspace`: Pico initializes host extensions but leaves the live workspace and agent counts at zero until the user opens a workspace or catalog session from the sidebar. Explicit session CLI options keep the normal initial-directory sequence. Opening a workspace does not create a main agent; the sidebar and `/cd` create one when needed.
