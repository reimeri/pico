...
- A second Esc while that cancel is still outstanding **force-cancels**: the UI goes idle immediately and the worker is abandoned. The tool function may keep running in the background until it returns. Reload/F5 still waits until that abandoned worker finishes so your code is not `dlclose`d underneath it. Do not use your own condition variable to wait for UI; Pico cannot wake it.
...
- A queued reload refuses new external turns and `subagent` delegations. Work already in a turn drains through its tool/model follow-ups before registrations change.
