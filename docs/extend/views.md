...
- `PICO_SLOT_SIDEBAR` — left column. Builtin `sidebar` lists workspaces and normal sessions. The column exists only if at least one view is registered here.
...
- `PICO_SLOT_OVERLAY` — drawn after the shell (warnings, popups, modals). Builtin `/extensions`, `/show-prompt`, the add-workspace modal, and the `ask_user` questionnaire are overlay modals.
...
- `PicoUi_ModalOpen` is true while `/extensions` or `/show-prompt` is open, a footer menu is open, the add-workspace modal is open, or a tool ask is pending in the active session tree. Builtin composer, chat, and footer skip input then. A custom ask overlay still receives pointer hits; it may consume `GetCharPressed()` from `on_frame` after the composer has skipped.
...
- A queued reload keeps ask/cancel UI responsive while blocking new submits. Do not cache active-agent transcript indices across session switches; borrowed messages become stale after a pump, mutation, or close.
