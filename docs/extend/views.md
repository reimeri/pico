...
- `PICO_SLOT_OVERLAY` — drawn after the shell (warnings, popups, modals). Builtin `/extensions`, `/show-prompt`, the `ask_user` questionnaire, and the subagent inspect chat are overlay modals.
...
- `PicoUi_ModalOpen` is true while `/extensions` or `/show-prompt` is open, a footer menu is open, a subagent inspect chat is open, or a tool ask is pending. Builtin composer, chat, and footer skip input then. A custom ask overlay still receives pointer hits; it may consume `GetCharPressed()` from `on_frame` after the composer has skipped.
