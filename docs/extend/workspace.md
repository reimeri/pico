...
Once quiescent, Pico compiles and initializes a complete candidate module/registration generation, then publishes it atomically. Compile, load, validation, or init failure leaves the previous generation active. An accepted turn retains the generation it started with until that turn and every event, pending call, ask, delegation reference, and callback from it is gone.
