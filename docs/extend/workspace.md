...
Closing a workspace is backend-only (`pico_workspace_request_close`). The builtin sidebar is the workspace-management UX: it lists the **disk catalog** under `~/.config/pico/sessions/` (each encoded-key folder, with `.workspace.json` for `path` / display name / order / collapsed and a stat-generation-keyed session listing cache, while parent `*.jsonl` files remain authoritative sessions). Catalog mutations are serialized per workspace, and scans re-parse only new or changed JSONL files. Scan omits an ordinary catalog workspace whose `path` is not an existing directory. A recorded linked-worktree catalog remains visible as unavailable so its sessions cannot silently route to the main checkout. A catalog entry is not a live `PicoWorkspace` until the user opens it. Normal CLI startup opens cwd, creates one main agent, and ensures cwd has a catalog folder. The installed desktop launcher starts with zero live workspaces; opening a catalog workspace/session or using the sidebar Projects `+` creates the live workspace and its first main agent. Catalog size is not capped at 8; opening a ninth live workspace still returns `PICO_LIMIT` (“Too many workspaces are open.”). The sidebar's project rows replace the folder icon with a pen on hover; clicking the pen opens an anchored project-settings popover (the rest of the row still collapses/expands sessions). Rename changes only the project-group display label, never the checkout path. Stash moves the group to a bottom **Stashed** section, collapsed by default on startup; sessions remain browsable and resumable there, and opening `/cd` or a session does not restore it. Restore moves it back to Projects. These actions operate on the logical Git project group, including its linked checkouts. Delete saved history requires confirmation and is blocked while any agent in that group is live in the current process (deletion does not take the inter-process catalog locks and is not coordinated with other Pico processes); it removes session files, catalog metadata, and media for the composer and listed parent sessions in every checkout of that group — subagent session media may remain — but never removes a checkout directory or unrelated project files. An explicitly reopened deleted project starts a fresh catalog entry. Project display name and stash state live in project-level catalog metadata, separate from checkout-local `.workspace.json`; the latter remains the authoritative checkout-to-session mapping. Project deletion leaves lock bookkeeping in place rather than unlinking a lock another process might hold. `/cd` never closes the previous workspace.
